...
namespace QCloud.CosApi.Api
{
    public class CosCloud
    {
        ...
        public CosCloud(int _appId, string _signature, int timeOut = HTTP_TIMEOUT_TIME)
        {
            this.appId = _appId;
        ...
        /// <returns></returns>
        public string UploadFile2(string bucketName, string remotePath, string fileName, byte[] fileContent, Dictionary<string, string> parameterDic = null)
        {
            ...
            {
                return Upload2(bucketName, remotePath, fileName, fileContent, bizAttribute, insertOnly);
            }
            else
            {
                //throw new Exception("图片大小超过8M");
                //分片上传
                int sliceSize = SLICE_SIZE.SLIZE_SIZE_3M;
                ...
                return SliceUploadFile2(bucketName, remotePath, fileName, fileContent, bizAttribute, slice_size, insertOnly);
            }
        }
        ...
        /// <returns></returns>
        public string Upload2(string bucketName, string remotePath, string fileName, byte[] fileContent,
                                  string bizAttribute = "", int insertOnly = 1)
        {
            var url = generateURL(bucketName, remotePath);
            var sha1 = SHA1.GetFileSHA1_new(fileContent);
            ...
            var sign = this.isSign ? this.signature : Sign.Signature(appId, secretId, secretKey, expired, bucketName);
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return httpRequest.SendRequest2(url, ref data, HttpMethod.Post, ref header, timeOut, fileName, fileContent);
        }
        ...
        /// <param name="insertOnly">是否覆盖同名文件</param>
        /// <returns></returns>
        public string SliceUploadInit2(string bucketName, string remotePath, string fileName, byte[] fileContent, string fileSha,
                                       string bizAttribute = "", int sliceSize = SLICE_SIZE.SLIZE_SIZE_1M, int insertOnly = 1)
        {
            ...
            data.Add("insertOnly", insertOnly);
            string uploadParts = computeUploadParts2(fileName, fileContent, sliceSize);
        ...
        private string computeUploadParts2(string fileName, byte[] fileContent, int sliceSize)
        {
            try
            {
                ...
                Stream oFileStream = new MemoryStream(fileContent);
                CosSha1Pure sha = new CosSha1Pure();
                StringBuilder jsonStr = new StringBuilder();

                jsonStr.Append("[");

                for (int i = 0; offset < fileContent.Length; ++i, offset += readLen)
                {
                    oFileStream.Seek(offset, SeekOrigin.Begin);
                    readLen = oFileStream.Read(buffer, 0, sliceSize);
                    totalLen += readLen;
                    string dataSha;

                    sha.HashCore(buffer, 0, readLen);
                    if ((readLen < sliceSize) || (readLen == sliceSize && totalLen == fileContent.Length))
                    {
                        //最后一片
        ...
        /// <returns></returns>
        public string SliceUploadData2(string bucketName, string remotePath, string fileName, byte[] fileContent, string fileSha, string session, long offset, int sliceSise, string sign)
        {
            var url = generateURL(bucketName, remotePath);
            var data = new Dictionary<string, object>();
            ...
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return httpRequest.SendRequest2(url, ref data, HttpMethod.Post, ref header, timeOut, fileName, fileContent, offset, sliceSise);
        }
        ...
        /// <param name="session">init请求返回的session</param>
        /// <returns></returns>
        public string SliceUploadFinish2(string bucketName, string remotePath, string fileName, byte[] fileContent, string fileSha, string session)
        {
            var url = generateURL(bucketName, remotePath);
            var fileSize = fileContent.Length;
            var data = new Dictionary<string, object>();
        ...
        /// <param name="bizAttribute"></param>
        /// <param name="insertOnly"></param>
        /// <returns></returns>
        public bool DownloadFile(string bucketName, string url, string savePath, string bizAttribute = "", int insertOnly = 1)
        {
            var expired = getExpiredTime();
            var sign = Sign.Signature(appId, secretId, secretKey, expired, bucketName);
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return httpRequest.SendRequest(url, savePath, ref header, timeOut);
        }
        ...
        {
            return Extension.ToUnixTime(DateTime.Now) / 1000 + SIGN_EXPIRED_TIME;
        }
