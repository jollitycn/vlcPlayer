...
namespace JGNet.Common
{
    public class BarCodeEAN13
    {
        ...
        public static bool CheckBarCode(String orgBarCode)
        { 
            String barCode = orgBarCode;
        ...
        public static void Paint_EAN13(string Numbers12bit, Graphics g, Rectangle drawBounds)
        {
           ...
           Font font = new Font("Microsoft Sans Serif", 8);
            ...
            int lineWidth = 1;
            int x = drawBounds.Top;
            int y = drawBounds.Left;
            int x2 = y + drawBounds.Height;
           Pen blackPen = new Pen(Color.Black, lineWidth);
            Pen whitePen = new Pen(Color.White, lineWidth);
            SolidBrush brush = new SolidBrush(Color.Black);
            // Paint human readable 1 system symbol code
            g.DrawString(symbols[0].ToString(), font, brush, x, x2 - 16);
            x += 10;

            // Paint left 'guard bars', always same '101'
            g.DrawLine(blackPen, x, y, x, x2);
            x += lineWidth;
            g.DrawLine(whitePen, x, y, x, x2);
            x += lineWidth;
            g.DrawLine(blackPen, x, y, x, x2);
            x += lineWidth;
            ...
            g.DrawString(barCode.Substring(1, 6), font, brush, x, x2 - 12);
            ...
            for (int i = 0; i < xxx.Length; i++)
            {
                if (xxx[i] == '1')
                {
                    g.DrawLine(blackPen, x, y, x, x2 - 12);
                }
                else
                {
                    g.DrawLine(whitePen, x, y, x, x2 - 12);
                }
                x += lineWidth;
            }
            ...
            // Paint human readable left-side 6 symbol code
            g.DrawString(barCode.Substring(7, 6), font, brush, x, x2 - 12);
