...
namespace JGNet.Common.Core.Util
{
    public class ValidateUtil
    {
        ...
        /// 验证输入字符串是否超出限定长度
        /// </summary>
        /// <param name="s">验证的串</param>
        /// <param name="length">长度</param>
        /// <returns></returns>
        public static bool InputOutOfLimit(String s,String name, int length)
        {
            bool outOfLimit = System.Text.Encoding.Default.GetBytes(s).Length > length;
            if (outOfLimit) { GlobalMessageBox.Show(name+"不能超过" + length + "个字符，" + length / 2 + "个中文！"); };
            return outOfLimit;
        }

        public static bool CheckNumber(string money, decimal minNum, decimal maxNum)  //验证输入字符串是否是数字（包括小数）
        {
            bool flag = true;
            try
            {
                ...
                {
                    flag = false;
                    GlobalMessageBox.Show("请输入金额！", "提示");
                }
                else
                {
                    if (money == "-")
                    ...
                    decimal checkValue = Decimal.Parse(money);
                    if (minNum > checkValue || checkValue > maxNum)
                    {
                        //超出范围
                        GlobalMessageBox.Show("输入超出范围！");
                        flag = false;
                    }
                }
                return flag;
            ...
            {
                CommonGlobalUtil.WriteLog(ex.Message);
                //转换异常
                GlobalMessageBox.Show("输入数字的格式不正确！");
                flag = false;
            }
            finally
            {
        ...
        public static String ZeroToEmpty(String obj, bool reportShowZero)
        {
            if (reportShowZero)
            {
                return obj;
            ...
            {
                return ZeroToEmpty(obj);
            }
        }
        private static String ZeroToEmpty(String obj)
        {
        ...
        public static String CheckEmptyValue(object obj)
        {
            if (obj != null)
            {
                return obj.ToString();
        ...
        public static object GetNumericType(Type dataType)
        {
            object obj = null;

            if (dataType == typeof(int))
            {
                obj = decimal.ToInt32(0);
            }
            else if (dataType == typeof(double))
            {
                obj = decimal.ToDouble(0);
            }
            else if (dataType == typeof(long) ||
                        dataType == typeof(Int64))
            {
                obj = decimal.ToInt64(0);
            }
            else if (dataType == typeof(double))
            {
                obj = decimal.ToDouble(0);
            ...
            {
                obj = decimal.ToSingle(0);
            }
            else if (dataType == typeof(Byte))
            {
                obj = decimal.ToByte(0);
            }
            return obj;
        }
        public static String CheckNotNullValue(object obj)
        {
            if (obj != null)
            {
                return obj.ToString();
        ...
        public static bool CheckMoney(string money)  //验证输入字符串是否是数字（包括小数）
        {
            bool flag = true;
            int count = 0;
            if (money.Length == 0)
            {
                flag = false;
                GlobalMessageBox.Show("请输入数字！", "提示");
            }
            else
            {
